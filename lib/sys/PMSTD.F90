...
CONTAINS
 ...
 PURE FUNCTION PM__FMT_INT(I,N) RESULT(S)
   INTEGER,INTENT(IN):: I
   INTEGER(PM__LN),INTENT(IN):: N
   TYPE(PM__STR):: S
   CHARACTER(LEN=16):: BUFFER
   CHARACTER(LEN=ABS(N)):: SBUFFER
   INTEGER:: AN
   AN=ABS(N)
   WRITE(BUFFER,'(I10)') I
   BUFFER=ADJUSTL(BUFFER)
   IF(LEN_TRIM(BUFFER)>AN) THEN
      SBUFFER=REPEAT('*',AN)
   ELSE
      SBUFFER=BUFFER
      IF(N>0) SBUFFER=ADJUSTR(SBUFFER)
   ENDIF
   S=PM__STRVAL(SBUFFER)
 END FUNCTION PM__FMT_INT
 ...
 END FUNCTION PM__LONG_TO_STR
  ...
  PURE FUNCTION PM__FMT_LONG(I,N) RESULT(S)
   INTEGER(PM__LN),INTENT(IN):: I
   INTEGER(PM__LN),INTENT(IN):: N
   TYPE(PM__STR):: S
   CHARACTER(LEN=24):: BUFFER
 ...
 PURE FUNCTION PM__FMT_OFFSET(I,N) RESULT(S)
   INTEGER(PM__LLN),INTENT(IN):: I
   INTEGER(PM__LN),INTENT(IN):: N
   TYPE(PM__STR):: S
   CHARACTER(LEN=34):: BUFFER
   CHARACTER(LEN=N):: SBUFFER
   INTEGER:: AN
   AN=ABS(N)
   WRITE(BUFFER,'(I30)') I
   BUFFER=ADJUSTL(BUFFER)
 ...
 END FUNCTION PM__INT64_TO_STR
  ...
  PURE FUNCTION PM__FMT_INT64(I,N) RESULT(S)
   INTEGER(PM__I64),INTENT(IN):: I
   INTEGER(PM__LN),INTENT(IN):: N
   TYPE(PM__STR):: S
   CHARACTER(LEN=24):: BUFFER
 ...
 PURE FUNCTION PM__FMT_REAL(R,N) RESULT(S)
   REAL,INTENT(IN):: R
   INTEGER(PM__LN),INTENT(IN):: N
   TYPE(PM__STR):: S
   CHARACTER(LEN=16):: BUFFER,FMT
   CHARACTER(LEN=ABS(N)):: SBUFFER
   INTEGER:: AN
   AN=ABS(N)
   WRITE(FMT,'("(G15.",I2,")")') MAX(1,MIN(AN-6,10))
   WRITE(BUFFER,FMT=FMT) R
   BUFFER=ADJUSTL(BUFFER)
   IF(LEN_TRIM(BUFFER)>AN) THEN
      SBUFFER=REPEAT('*',AN)
   ELSE
   ...
   ENDIF
   S=PM__STRVAL(SBUFFER)
 END FUNCTION PM__FMT_REAL

 PURE FUNCTION PM__FMT_REAL_DP(R,N,M) RESULT(S)
   REAL,INTENT(IN):: R
   INTEGER(PM__LN),INTENT(IN):: N,M
   TYPE(PM__STR):: S
   CHARACTER(LEN=16):: BUFFER,FMT
   CHARACTER(LEN=ABS(N)):: SBUFFER
   INTEGER:: AN
 ...
 PURE FUNCTION PM__FMT_DOUBLE(R,N) RESULT(S)
   DOUBLE PRECISION,INTENT(IN):: R
   INTEGER(PM__LN),INTENT(IN):: N
   TYPE(PM__STR):: S
   ...
   INTEGER:: AN
   AN=ABS(N)
   WRITE(FMT,'("(G25.",I2,")")') MAX(1,MIN(AN-6,10))
   WRITE(BUFFER,FMT=FMT) R
   BUFFER=ADJUSTL(BUFFER)
   IF(LEN_TRIM(BUFFER)>AN) THEN
      SBUFFER=REPEAT('*',AN)
   ...
   ENDIF
   S=PM__STRVAL(SBUFFER)
 END FUNCTION PM__FMT_DOUBLE

 PURE FUNCTION PM__FMT_DOUBLE_DP(R,N,M) RESULT(S)
   DOUBLE PRECISION,INTENT(IN):: R
   INTEGER(PM__LN),INTENT(IN):: N,M
   TYPE(PM__STR):: S
   CHARACTER(LEN=32):: BUFFER,FMT
   ...
   ENDIF
   S=PM__STRVAL(SBUFFER)
 END FUNCTION PM__FMT_DOUBLE_DP
 ...
 PURE FUNCTION PM__FMT_STRING(S,N) RESULT(S2)
   TYPE(PM__STR),INTENT(IN):: S
   INTEGER(PM__LN),INTENT(IN):: N
   TYPE(PM__STR):: S2
   CHARACTER(LEN=ABS(N)):: STR
   INTEGER:: AN,DIFF,LS
   AN=ABS(N)
   LS=LEN(S%P)
   IF(LS>AN) THEN
      STR=REPEAT('*',AN)
   ELSEIF(LS==AN.OR.N<0) THEN
      STR=S%P
   ELSEIF(N>0) THEN
      DIFF=AN-LS
      STR(1:DIFF)=' '
 ...
 RECURSIVE SUBROUTINE PM__GET_MPI_DISP_TYPE(TNO,OFF,M,MPI_TYP)
   ...
   INTEGER,INTENT(OUT):: MPI_TYP
   INTEGER:: ERRNO
   INTEGER(PM__LN):: START,END,I,N,NOFF
   ...
   IF(NOFF>PM__MAX_MESSAGE_SIZE) THEN
      ...
      DO I=1,N
         ...
         CALL PM__GET_MPI_DISP_UNIT_TYPE(TNO,OFF(START:&
           END),M,TYPES(I))
      ENDDO
      CALL MPI_TYPE_CREATE_STRUCT(INT(N),LENS,DISPS,TYPES,MPI_TYP,ERRNO)
      DO I=1,N
 ...
 SUBROUTINE PM__GET_MPI_SUBRANGE_TYPE(TNO,START,END,STEP,WIDTH,ALIGN,TNO2)
   INTEGER,INTENT(IN):: TNO
   INTEGER(PM__LN),INTENT(IN):: START,END,STEP,WIDTH,ALIGN
   INTEGER,INTENT(OUT):: TNO2
   INTEGER(PM__LN):: N
   INTEGER(MPI_COUNT_KIND):: LB,SIZ
   INTEGER(MPI_ADDRESS_KIND):: DISPL(3)
   INTEGER:: BLK(3),MTYPES(3),ERRNO,M,TNO3
   !write(*,*) '#',TNO,START,'..',END,STEP,WIDTH,ALIGN
